...
# ---------------------------------------------------------------------------
# Module-level helpers (moved from runtime.py — see ADR-019 Phase 7 S9)
# ---------------------------------------------------------------------------
...
def _normalise_imu_window_response(
    ...
    *,
    ...
    """Project ``ImuWindowResponse`` into :class:`AIPrediction`.

    Kept here (alongside the FallService) rather than in runtime.py so
    the full fall AI path lives in one place.  runtime.py retains a
    re-export (``from api_server.services.fall_service import
    _normalise_imu_window_response``) for any legacy reference.
    """
