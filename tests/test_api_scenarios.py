...
class TestApiScenarios(unittest.TestCase):
    ...
    def test_list_scenarios(self) -> None:
        response = self.client.get("/api/v1/sim/scenarios")
    ...
    def test_apply_scenario_changes_vitals_profile(self) -> None:
        device = self.client.post("/api/v1/sim/devices", json={"name": "Scenario Device", "type": "smartwatch"}).json()
        session = self.client.post("/api/v1/sim/sessions", json={"device_ids": [device["id"]], "speed": 1}).json()
        self.client.post(f"/api/v1/sim/sessions/{session['id']}/start")

        normal = self.client.get("/api/v1/sim/vitals/latest", params={"deviceId": device["id"]}).json()

        apply_hypoxia = self.client.post(
            "/api/v1/sim/scenarios/apply",
            json={"device_id": device["id"], "scenario_id": "hypoxia_critical"},
        )
        self.assertEqual(apply_hypoxia.status_code, 204)

        hypoxia = self.client.get("/api/v1/sim/vitals/latest", params={"deviceId": device["id"]}).json()
    ...
    def test_push_sleep_for_date_rejects_today_and_future(self) -> None:
        ...
        device = self.client.post("/api/v1/sim/devices", json={"name": "Sleep API Device", "type": "smartwatch"}).json()

        today_response = self.client.post(
            "/api/v1/sim/scenarios/sleep/push-date",
    ...
    def test_push_sleep_for_date_rejects_older_than_one_year(self) -> None:
        today = datetime.now(timezone.utc).date()
        device = self.client.post("/api/v1/sim/devices", json={"name": "Sleep API Device 2", "type": "smartwatch"}).json()
    ...
    def test_push_sleep_for_date_returns_runtime_result(self) -> None:
        runtime = get_runtime()
        device = self.client.post("/api/v1/sim/devices", json={"name": "Sleep API Device 3", "type": "smartwatch"}).json()
    ...
    def test_get_sleep_history_returns_runtime_rows(self) -> None:
        runtime = get_runtime()
        device = self.client.post("/api/v1/sim/devices", json={"name": "Sleep API Device 4", "type": "smartwatch"}).json()
        ...
        try:
            response = self.client.get(
                "/api/v1/sim/analytics/sleep/history",
...
class TestBackfillEndpoint(unittest.TestCase):
    ...
    def _make_device(self) -> str:
        return self.client.post(
            "/api/v1/sim/devices", json={"name": "Backfill Test Device", "type": "smartwatch"}
        ).json()["id"]
