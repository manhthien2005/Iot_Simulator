...
try:
    ...
    from Iot_Simulator.api_server.dependencies import SessionRecord, SimulatorRuntime, reset_runtime_for_tests
...
class TestApiDashboard(unittest.TestCase):
    ...
    def test_dashboard_summary(self) -> None:
        response = self.client.get("/api/sim/dashboard/summary")
    ...
    def test_registry_status(self) -> None:
        response = self.client.get("/api/sim/registry/status")
        self.assertEqual(response.status_code, 200)
        payload = response.json()
    ...
    def test_recent_events_endpoint(self) -> None:
        self.client.post("/api/sim/events", json={"device_id": self.device_id, "event_type": "device_offline"})
        response = self.client.get("/api/sim/events/recent?limit=5")
        self.assertEqual(response.status_code, 200)
